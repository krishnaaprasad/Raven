...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    async function fetchOrder() {
      try {
        const res = await fetch(`/api/payment/verify?orderId=${orderId}&cfOrderId=${cfOrderId}`);
        ...
        if (data.success && data.paid) {
          ...
          const failedOrder = {
            ...
            shipping: JSON.parse(localStorage.getItem('checkoutShipping')) || 'standard',
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-[#e6decf] p-10 max-w-md w-full animate-fadeIn relative z-10">
      <div className="w-20 h-20 mx-auto mb-6 flex items-center justify-center rounded-full bg-[#D4AF37]/20 border-4 border-[#D4AF37] animate-bounce-slow">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="#D4AF37" className="w-10 h-10">
          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
        </svg>
      </div>
      <h1 className="text-3xl font-bold mb-3 text-[#6B4B1E] tracking-wide">Thank You for Your Purchase!</h1>
      <p className="text-[#8B6C3A] mb-6">Your order has been placed successfully. We’ll send you a confirmation email soon.</p>
      <div className="text-left bg-[#FCF8F3] rounded-xl p-4 border border-[#e6decf] mb-4">
        <p className="text-[#6B4B1E] mb-1"><strong>Order ID:</strong> {orderId}</p>
