...
export function getReviewPayload(order, item) {
  ...
  // CTA dynamic part — the template URL is: https://www.ravenfragrance.in/product/{{1}}
  // We send just the slug + query params to replace {{1}}
  const ctaSuffix = `${productSlug}?review=true`;
  ...
  return {
    ...
    ctaVariables: [ctaSuffix], // CTA 1 = slug?review=true
  };
}
