...
export function getDeliveryPayload(order) {
  ...
  // Product image for header — handle both string and object formats
  const rawImage = firstItem?.image || "";
  const headerImage = typeof rawImage === "string"
    ? rawImage
    : rawImage?.original || rawImage?.thumbnail || "";
...
export function getReviewPayload(order, item) {
  ...
  // Header image — handle both string and object formats
  ...
  const reviewSlug = `${productSlug}?review=true#reviews-section`;
