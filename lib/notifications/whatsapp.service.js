...
async function sendTemplateRequest({
  ...
  // CTA dynamic URL variables
  // Message Central uses: button_url_0_1, button_url_0_2, etc.
  // For templates with multiple buttons, try both index patterns
  if (ctaVariables.length > 0) {
    ctaVariables.forEach((url, idx) => {
      if (url) {
        form.append(`button_url_0_${idx + 1}`, String(url));
      }
    });
  }

  console.log("📤 WhatsApp API Request:", {
    url: `${WHATSAPP_API_BASE}/broadcasts/single`,
    projectId,
    templateId,
    phone,
    bodyVariables,
    headerImageUrl: headerImageUrl ? "SET" : "NONE",
    ctaVariables,
  });
